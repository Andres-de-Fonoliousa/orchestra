...
def cmd_upgrade():
    ...
    print("Orchestra version:", version)
    print()
    logs = run_git(["log", "--oneline", "-5"], brain / "memory")
    if logs and logs.returncode == 0:
        print("Memory git history:")
        print(logs.stdout)
    print()
    print("Upgrade blueprint lives in the agent_orchistration project:")
    print("  docs/ROADMAP.md  (v2.0: sqlite history, auto-journal plugin, sync)")
    print("  v2.0 steps: `python orchestra.py upgrade` once that module ships.")
...
<main>
  ...
  <section class="card"><h2>Quick manual</h2>
    ...
    <code>orchestra serve</code> — this page · <code>doctor</code> · <code>commit</code> · <code>handoff</code> · <code>upgrade</code></p>
...
def main():
    ...
    sub.add_parser("upgrade", help="show upgrade status for v2.0")
